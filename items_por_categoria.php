...
if (isset($_GET['categoria'])) {
    ...
    $query = $pdo->prepare('SELECT * FROM Producto WHERE Categoria_ID = ?');
    $query->execute([$categoria_id]);
    $productos = $query->fetchAll(PDO::FETCH_OBJ);
    
    // Obtiene el nombre de la categoría
    $categoria = obtenerCategoria($categoria_id);
} else {
    // Maneja el caso si no se proporciona un ID de categoría
